import React from "react";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faMinus, faClose ,faPlus} from "@fortawesome/free-solid-svg-icons";
import { useDispatch } from "react-redux";
import { addOne, removeFromCart, removeOne } from "../../redux/cart";

import classes from "./CartItem.module.scss";

const CartItem = ({ image, name, id, quantity, price }) => {
  const dispatch = useDispatch();
  const handleRemove = () => dispatch(removeFromCart(id));
  const handlePlus = () => dispatch(addOne(id));
  const handleMinus = () => dispatch(removeOne(id));
  return (
    <div className={classes["cart"]}>
      <img
        className={classes["cart__image"]}
        src={image}
        alt={name}
      />
      <h5 className={classes["cart__name"]}>{name}</h5>
      <div className={classes["cart__controls"]}>
        <button onClick={handleMinus} className={classes["cart__minusBtn"]}>
          <FontAwesomeIcon icon={faMinus} />
        </button>
        <p className={classes["cart__amount"]}>{quantity}</p>
        <button onClick={handlePlus} className={classes["cart__plusBtn"]}>
        <FontAwesomeIcon icon={faPlus} />
        </button>
      </div>
      <p className={classes["cart__price"]}>{price} $</p>
      <button onClick={handleRemove} className={classes["cart__clear"]}>
        <FontAwesomeIcon icon={faClose} />
      </button>
    </div>
  );
};

export default CartItem;
